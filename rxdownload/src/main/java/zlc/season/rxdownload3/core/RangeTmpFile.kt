...
class RangeTmpFile(val mission: RealMission) {
    ...
    fun checkFile() {
        logd("check file")
        if (file.exists()) {
            logd("totalSize ${fileStructure.totalSize} mission.totalSize ${mission.totalSize}")
    ...
    fun getLastModified():Long{
        if (file.exists()){
            readStructure()
            return fileStructure.lastModified
        }
        return 0L
    }
    ...
    fun currentStatus(): Status {
        ...
        val lastModified = fileStructure.lastModified
...
//            return FILE_HEADER_MAGIC_NUMBER_HEX.size() + 16L
        ...
        fun readSegments(source: BufferedSource) {
            ...
            for (i in 0 until totalSegments) {
                ...
                logd("index $index start $start current $current end $end")
