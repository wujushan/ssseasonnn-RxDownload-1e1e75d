...
class RealMission(val actual: Mission, val semaphore: Semaphore) {
    ...
    fun setLastModified(response: Response<Void>): RealMission {
        val lastModified = lastModified(response)
        this.actual.lastModified = lastModified
        return this
    }
}